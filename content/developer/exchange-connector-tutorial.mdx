...
An order could be filled by other market participants over time, once it's live on an exchange.
Depending on the order types, i.e. limit or market, the order could be filled either immediately or after it is fulfilled by another market participant.

For every order fill on our orders, whether partially or fully, the exchange connector must emit a `OrderFilledEvent`, to notify the strategy modules about the progress of the order.
...
If an order has failed or has been rejected for any reason other than cancellation or expiry, then a `MarketOrderFailureEvent` must be emitted.
