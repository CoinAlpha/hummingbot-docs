...
<Callout
  ...
  link={[
    "https://github.com/CoinAlpha/hummingbot/blob/master/hummingbot/connector/exchange/bittrex/bittrex_api_order_book_data_source.py",
  ]}
/>
...
<Callout
  type="note"
  body="Might require `convert_[snapshot/diff]_message_to_order_book_row` from the `ActiveOrderTracker` to convert the messages into `OrderBookRow`"
/>
...
<Callout
  type="note"
  body="You may be required to call `start` in the base class by using `await super().start()`. This is #optional# as long as there is a task listening for trade messages and emitting the `TradeEvent` as seen in `c_apply_trade` in [`OrderBook`]"
  link={[
    "https://github.com/CoinAlpha/hummingbot/blob/master/hummingbot/core/data_type/order_book.pyx",
  ]}
/>
...
<Callout
  type="note"
  body="This is to be overridden and called by running `super().start()` in the custom implementation of `start`."
/>
