---
title: "Task 1 — OrderBookTracker & Data Source"
---
...
This function simply returns the appropriate exchange name
...
Performs the necessary API request(s) to get last traded price for the given markets (trading_pairs) and return a dictionary of `trading_pair` its last traded price.
...
The `OrderBook` is an interface class contains all the necessary functions and variables to maintain an order book of the specified trading pair.
...
Converts json snapshot data from the exchangee into standard `OrderBookMessage` format.
...
Mainly used for backtesting, it converts a snapshot data from the db into standard `OrderBookMessage` format.

**Input Parameter(s):** record: `RowProxy`, metadata: `Optional[Dict] = None` <br/>
**Expected Output(s):** `OrderBookMessage`
...
Mainly used for backtesting, it converts a diff data from the db into standard `OrderBookMessage` format.
...
Converts json trade data from the exchangee into standard `OrderBookMessage` format.
...
Mainly used for backtesting, it converts a trade data from the db into standard `OrderBookMessage` format.
...
Integrating a new exchange connector will required implementing its own class that extends that extends from `OrderBookMessage`.
...
Contains any miscellaneous functions that are not completely associated to the classes defined in the overall architecture but nonetheless necessary for Hummingbot to perform its functions adequately.
...
Contains all the constant variables associated with an exchange
...
For reference, of what could/should be included in the `*_constants.py` click [here](https://github.com/CoinAlpha/hummingbot/blob/master/hummingbot/connector/exchange/probit/probit_constants.py)
