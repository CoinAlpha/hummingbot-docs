---
title: "Task 2 — UserStreamTracker & Authentication"
---
...
Implementing an exchange connector would require you create its own data source that extends from the `UserStreamTrackerDataSource` base class here.
...
A property function that simply retrieves the timestamp(in ms) of the last user data message received by the websocket client.
...
Subscribes to all relevant user channels via web socket, and keep the connection open for incoming messages.<br/>
...
2. **WebSocket API**
   ...
   When an exchange does have WebSocket API support to retrieve user account details and order statuses, it would be ideal to incorporate it into the Hummingbot client when managing account balances and updating order statuses.
   This is especially important since Hummingbot needs to knows the available account balances and order statuses at all times.
...
Tracking a user's orders, trades and balance generally requires authentication tied to each request or a websocket connection.
...
The `Auth` class is responsible for createing the necessary request headers and/or data bodies necessary to authenticate said request/websocket connection.
...
As part of the Code Review and QA process, for each tasks(Task 1 through 3) you are **required** to include the unit test cases for the code review process to begin.
