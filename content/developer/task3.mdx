...
<Callout
  type="note"
  body="1. These checks are performed in `execute_buy` and `execute_sell` **before** placing an order. 2.In the event where the orders proposed by the strategies fail to meet the requirements as defined in the trading rules, the intended behaviour for the `Market` class is simply to raise an error and not place the order. 3.You may be required to add additional functions to help determine if an order meets the necessary requirements. "
/>
