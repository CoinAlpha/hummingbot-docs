...
The actual price vs limit order price (requested price). The actual definition of a limit order is to buy or sell at a specific price or better. The market moves pretty dynamically, so when we execute a trade, if we can get a better price than the one you asked for in your limit price, we pass on the price improvement.
