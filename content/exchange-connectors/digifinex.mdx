...
Users may experience websocket error and timeout may occur every 2 hours. There is a known server side issue with regards to their rate limit, where users might be prematurely banned for exceeding rate limits. For more details on trading rules click [here](https://docs.digifinex.com/en-ww/v3/#digifinex-api-trading-rules).

### Minimum order sizes

Minimum order size varies per market. It's the exchange's allowed All minimum trade quantities can be found in the following public API:
...
The data fields you should focus on are `min_volume` which refers to the minimum order size based on base asset and `min_amount` which is based on quote asset.
