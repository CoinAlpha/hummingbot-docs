...
To use [HitBTC](https://hitbtc.com/) connector, you will need to generate and provide your API key in order to trade using Hummingbot.
...
<Callout
  type="tip"
  body="You must enable 2FA in yur HitBTC account to create the API key. [How to enable 2FA]?"
  link={[
...
4. Enable the permissions for `Order book, History, Trading balance` and `Place/Cancel orders`. 2FA code is needed for enabling permissions.

![](/img/hitbtc-2fa.PNG)

![](/img/hitbtc-api-permission.png)

5. Now you have created an API key, connect it to Hummingbot by using the `connect` command.
...
HitBTC does not currently offer a subscript-able balance websockets feed, which results in balances only updated when they are polled leading to a slight delay in creating orders after cancelling.
