...
ProBit is a fast-growing exchange coming from South Korea. Its key characteristics include having a professional team, offering a great user experience with a global approach towards crypto trading, and a robust trading platform.
...
To use [ProBit](https://www.probit.com/) connector, you will need to generate and provide your API key in order to trade using Hummingbot.
...
1. Log in to your account at https://www.probit.com/, then click your My Page > API Management (If you do not have an account, you will have to create one and enable 2FA.)
...
<Callout
  type="warning"
  body=" For API key permissions, we recommend using `read` and `trade` enabled API keys; enabling `withdrawal` `or the equivalent is unnecessary `for current Hummingbot strategies."
/>
...
Minimum order size for ProBit exchange is 1 USDT.
...
- [Fees](https://support.probit.com/hc/en-us/articles/360017844972-Trading-Fee-Structure-at-ProBit#:~:text=Trading%20Fees,gives%20enhanced%20bonuses%20as%20well.)
