...
For the rate oracle conversion to show after the `balance`, `pnl`, `open_orders`, `trades`, and `status` command, you have to manually configure the settings in **config_global.yml**.

To change the `rate_oracle_source`, `global_token` and `global_token_symbol`, you can edit it by running the `config` command.
