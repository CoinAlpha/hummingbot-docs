...
<Callout
  ...
  link={["http://discord.hummingbot.io/"]}
/>
...
Pull Hummingbot ARM image when asked what version to use, you would need to build one for Raspberry Pi. 
```
...
coinalpha/hummingbot:dev-0.33.0-arm_beta
```

<Callout
  type="note"
  body="On the example above, `dev-0.33.0-arm_beta` was already available. For every release, you will need to create an Hummingbot ARM image so you can update your bot to the latest version."
/>
...
```
Log out and Log back in to enable `conda`, this will make the `conda` command available in shell / terminal.
...
## Build Hummingbot ARM image

When using Raspberry Pi, you would need to create an image for docker. First, you would need to install Hummingbot from source and run this command:
```
docker build -t coinalpha/hummingbot:INSERT-NAME-OF-IMAGE -f Dockerfile.arm .
```

![](img/docker_image.PNG)

The name of the Hummingbot ARM image will be use when being prompted to enter the Hummingbot version on `./create.sh`
On thes example above, you would need to run **v036**.
