...
Updating to the latest docker image (e.g. `coinalpha/hummingbot:latest`) when updating make sure to stop all the containers that are using the same image first before running the `./update.sh` script.
