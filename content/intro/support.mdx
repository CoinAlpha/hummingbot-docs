...
We recognize that running a trading bot for the first time is a big step for many new users, so we maintain 24/7 live support to assist our users. Before you reach out to our support, do look at some of the common issues and error messages found when using Hummingbot and how to resolve them.
