...
**Connector Status** column is an indicator if there are known issues with the connector or working correctly. More info in [Exchange Connector Status](/overview/#exchange-connector-status) and [Protocol connector Status](/overview/#protocol-connector-status).
