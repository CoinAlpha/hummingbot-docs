...
```
### start --restore
Allows you to start the bot, create order and set the previous order to hanging
...
Status check complete. Starting 'pure_market_making' strategy...
Restored 1 limit orders on binance...   

```


> **Notes** 
- This command will work when `exit --suspend` has been executed on the bot when closed.
- There are times that hanging orders will be cancelled when bot starts since it depends on `hanging_orders_cancel_pct`
...
### exit --suspend
Allows the bot to close without cancelling the limit order. This way when you run `start --restore` it will create a new limit order and set the previous order as hanging 
```
>>> exit --suspend
```
