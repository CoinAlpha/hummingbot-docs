...
<Callout type="note" body="`Help shape the direction of the Hummingbot codebase` In order to communicate the status of bug fixes and planned improvements, we recently began to publish the [Hummingbot Public Roadmap]. Help us prioritize stories by commenting on the ones you want, and feel free to submit new stories." link={["https://github.com/CoinAlpha/hummingbot/projects/2"]} />
...
<Callout type="note" body="Assets listed in the `balance` command are now listed in alphabetical order rather than in USD terms." />

## ☝️ Trade notifications

When the Hummingbot client makes a trade, it now appends a trade notification message to the lefthand output screen. Users of the [Telegram integration](/features/telegram/) should also see these notifications in the Telegram interface.
