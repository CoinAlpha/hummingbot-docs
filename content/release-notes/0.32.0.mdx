...
Hummingbot now supports [Bitfinex](https://www.bitfinex.com/) exchange! Bitfinex is a cryptocurrency exchange owned and operated by iFinex Inc., headquartered in Hong Kong and registered in the British Virgin Islands.
...
Read more about how to use Bitfinex connector [here](/spot-connectors/bitfinex/).

<Callout
  type="note"
  body="Bitfinex connector is currently not working with cross-exchange and arbitrage strategy."
/>
...
Read more about how to use Loopring connector [here](/spot-connectors/loopring/).

## 🎛 New pure market making feature: Order Override

Advanced users can now customize and specify how Hummingbot creates orders regarding order levels, spread and size.

Since this feature is for advanced users, it is not configurable from the Hummingbot client. Follow the guide in our documentation how to use the [Order Override](/strategies/order-override/#gatsby-focus-wrapper) feature.

## 📈 New price reference: Last Own Trade Price

We added `last_own_trade_price` to `price_type` in pure market-making to allow users to review their last trade price as a price reference when creating orders.

More information in [External Pricing Source Configuration](https://docs.hummingbot.io/strategies/external-price-source/).
...
<Callout
  type="note"
  body="Currently, `inventory_target_base_pct` can only be configured when running on centralized exchange connectors."
/>
...
Optional arguments for this command and sample usage can be found in our [documentation](/operation/command-ref/#ticker).
...
- A significant update on [Task 4. Required Connector Configuration](/developer/task4/#gatsby-focus-wrapper), most of these tasks are now contained within the connector package, you no longer need to update various other files in the Hummingbot.

## 🔧 Other Enhancements

- In arbitrage strategy, the `status` command displays the last calculated profitability [#2151](https://github.com/CoinAlpha/hummingbot/issues/2151)
