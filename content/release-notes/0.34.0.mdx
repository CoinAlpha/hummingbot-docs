...
The [`open_orders`](/operation/checking-status/#open_orders) command displays all current open (active) orders that the bots have placed across all markets. The [`pnl`](/operation/checking-status/#pnl) command lets users see all trade profits and lossses across all markets normalised in USD. And [`trades`](/operation/checking-status/#trades) command lets users see the trades across all markets in more detail.
