...
In v0.36, we added a new feature to the perpetual market making strategy. `stop_loss_spread` is now available with the `Profit_taking` and `Trailing_stop` feature.
...
![](/img/perpetual-history.png)

<Callout
  type="note"
  body="`history` for perpetual market making requires for the position to be closed to get the right `pnl` and return."
/>
