...
The idea of this strategy is to open a position in one direction and execute an order in the opposite direction on the spot. Learn more about it here #[2977](https://github.com/CoinAlpha/hummingbot/pull/2977).

## Improvement to Liquidity Mining strategy

In v0.36, we introduced the Liquidity Mining strategy. In v0.37, we made improvements to give users a better experience when using this strategy, we added two new parameters [`max_spread`](/strategies/liquidity-mining/#max_spread) and [`max_order_age`](/strategies/liquidity-mining/#max_order_age). Tables are also added to let you see more detail about your base and quote balance and also the campaign. Another improvement is better balance distribution on the selected `token`. Learn more about it here #[2961](https://github.com/CoinAlpha/hummingbot/pull/2961).

## Bug Fixes

- #[2949](https://github.com/CoinAlpha/hummingbot/pull/2949) Add `Exchanges Total` to `balance` command
  - **We want to thank community member [Iamtha](https://github.com/lamtha) for these contribution!**
...
- #[2989](https://github.com/CoinAlpha/hummingbot/pull/2989) Fix typo in `history` command
  - **We want to thank community member [krisklosterman](https://github.com/krisklosterman) for these contribution!**
