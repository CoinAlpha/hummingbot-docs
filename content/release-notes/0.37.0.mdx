...
In v0.36, we introduced the Liquidity Mining strategy. In v0.37, we made improvements give users a better experience when using this startegy, we added two new parameters has been added [`max_spread`](/strategies/liquidity-mining/#max_spread) and [`max_order_age`](/strategies/liquidity-mining/#max_order_age). Tables are also added to let you see more into detail about your balance and also the campaign. Another improvement is better balance distribution on the selected `token`.
