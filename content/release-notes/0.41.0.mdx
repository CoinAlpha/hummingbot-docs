...
We took our existing TWAP strategy (Time-weighted Average Price) from the development branch into production to be used in the latest version. This strategy allows users to execute either buy or sell orders continuously at a specified time interval.

## New Avellaneda Market Making Feature : Hanging Orders

The hanging orders feature that was previously available only in Pure Market Making strategy, is now added to Avellaneda Market Making. In addition to this feature, users can also aggregate hanging orders based on volume using the `hanging_orders_aggregation-type` parameter.

**Learn more on how to use this feature in this [documentation](/strategies/avellaneda-market-making).**
