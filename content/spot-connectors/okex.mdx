...
1. Log into your account at https://www.okex.com, then select **Account>API** (If you do not have an account, you will have to create one and verify your ID).
   <<<<<<< HEAD:content/exchange-connectors/okex.mdx
...
3. Follow on-screen instructions to create your API keys.

### OKEx upgrade to API v5

OKEx recently announce that they are upgrading their API from v3 to v5. Most of the newly created accounts are now integrated with API v5. Currently Hummingbot only works with API v3 and here is the workaround on how you can convert:

1. Create a new account on OKEx exchange
2. Go to telegram and look for `@OKExMarkets_bot`
3. Type `./start` then click `Start` now
4. Send the email address of your OKEx account
5. Account is being verified now, monitor your account and see if v3 is enabled.
...
> > > > > > > 647191f9e2011a278bfb792cfb96f5dbd26eeec2:content/spot-connectors/okex.mdx
...
Users can override the default fees by editing [`conf_fee_overrides.yml`](/operation-2/override-fees).
