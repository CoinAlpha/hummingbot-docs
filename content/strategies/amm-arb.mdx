...
- see [Setup Ethereum Wallet](https://docs.hummingbot.io/operation/keys-wallets-nodes/)
- see [Setup Infura Node](https://docs.hummingbot.io/operation/keys-wallets-nodes/). Take note of the Ethereum RPC URL to be use later for Gateway Docker settings.
...
- Make sure gateway is properly running together with the hummingbot client.
- You can initially setup the `rate_oracle_source` and test getting the rates before creating the AMM arbitrage strategy.

![Rate Oracle Amm Arb Config](img/rate-oracle-ammarb-config.png)

![Rate Oracle Amm Arb Status](img/rate_oracle_amm_arb_status.png)
