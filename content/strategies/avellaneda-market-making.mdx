...
The Volatility Sensibility will recalculate `gamma`, `kappa` and `eta` after the value of volatility sensibility threshold in percentage is achieved. When parameter is set to 0, it will recalculate `gamma`, `kappa`, and `eta` each time an order is created. The default value for the parameter is 20.
