...
8. (Optional) Enter `config paper_trade_enabled` to run paper trading first.
  > **Caution:** If the ethereum network is already set to MAIN_NET, paper trade only works on one direction on the exchange side.
9. Enter `start` to run the strategy.
10. To check transactions, you can use etherscan.io to check if any pending transaction gets stuck for too long (> 5 min). If any Tx is got stuck, change the `config ethgasstation_gas_level` to fast, the transaction setting should complete < 1-2 min
...
<div style={{ padding: "10px 20px", backgroundColor: "#1c1c1c" }}>
<font color="#20a26a">Enter the token trading pair you would like to trade on balancer (e.g. WETH-DAI)</font>
</div>
...
<div style={{ padding: "10px 20px", backgroundColor: "#1c1c1c" }}>
<font color="#20a26a">Enter the token trading pair you would like to trade on balancer (e.g. ZRX-ETH)</font>
</div>
...
<div style={{ padding: "10px 20px", backgroundColor: "#1c1c1c" }}>
<font color="#20a26a">What is the amount of [first trading pair base asset] per order?</font>
</div>
