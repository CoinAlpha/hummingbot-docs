...
Last price - is the most recent price that the asset have been traded
...
The bot will use the last price that one of your orders was taken as the price type
...
When `price_type` is set to `best_bid`, the price will be based on the besk bid in the orderbook.
...
When `price_type` is set to `best_ask`, the price will be based on the besk ask in the orderbook.
...
Hummingbot will calculate the average cost of your inventory every time a buy order is filled, and all sell orders will be created at `inventory_cost` + `ask_spread`.
...
<Callout
  type="tip"
  body="#Updated as of version 0.36.0#, added [inventory_cost] to price_type. When using inventory_cost, you would need to set inventory price before you start your Hummingbot using this command `config inventory_price`"
  link={[
    "/release-notes/0.36.0/#inventory-average-cost-added-as-a-new-price_type-configuration",
  ]}
/>
