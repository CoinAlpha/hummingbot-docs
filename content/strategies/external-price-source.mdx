...
Run `config price_type` command to change the price reference to `last_price`, `last_own_trade_price`, `best_bid`, `best_ask`, and `inventory_cost`.
...
<Callout
  ...
  link={["/release-notes/0.36.0/#inventory-average-cost-added-as-a-new-price_type-configuration"]}
/>
...
<Callout type="note" body="Currently, the external price source cannot be the same as the maker exchange (i.e. if the bot is trading on Binance, the `price_source_exchange` cannot be Binance)." />
