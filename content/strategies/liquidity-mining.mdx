...
Set the maximum spread for your orders. Spreads will not go higher if you set a value for max spread. By default, this parameter is set to -1.
...
The `max_order_age` parameter allows you to set a specific duration when to reset your order's age. Click [here](/strategies/max-order-age/) to learn more about this parameter.
...
These values will show after running `status` command. `Budget`, `Markets`, `Miner`, and `Orders`.
...
This shows the markets that you have entered to provide liquidity ALGO-USDT, AVAX-USDT, and FIRO-USDT as shown in the example above.
...
Shows the best bid of the market

`Best_ask`

Shows the best ask of the market
...
This shows the markets chosen, rewards for that certain campaign, rewards per week, current liquidity of that market, yield, and maximum spread for that campaign. This gives you insight on the campaigns you are participating to easily check what are the details for the campaigns.
