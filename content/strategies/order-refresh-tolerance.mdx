...
<Callout type="note" body="`Not enough balance for buy (sell) order .... order_amount is adjusted to` > `Not cancelling active orders since difference between new order prices and current order prices is within` is no longer shown in logs." />
