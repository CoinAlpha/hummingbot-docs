...
Perpetual market making allows Hummingbot users to market make on supported derivatives exchanges. Position management features are introduced to configure the bot further to make managing positions easier and remove the need to manually interact with the derivative exchange.
...
<Callout
  type="tip"
  body="Users can test how this strategy works without risking real funds by connecting their Binance Futures Testnet account to Hummingbot. We also recommend running the bot side-by-side with the exchange to help to learn more about the strategy."
/>
...
8. If you selected [`Profit_taking`](#profit-taking), enter the spreads of your profit taking orders to close a long or short position in `long_profit_taking_spread` `short_profit_taking_spread`
9. If you selected [`Trailing_stop`](#trailing-stop), enter the spread when you want the bot to start trailing as `ts_activation_spread` and the exit price as `ts_callback_rate`
...
If the orders are not filled when they reach `order_refresh_time`, they are cancelled and a new set of orders are created to ensure our orders are refreshed based on specified bid/ask spread.
...
If the buy order is filled then the opposite sell order is cancelled and a long position is opened. On the other hand, if your sell order is filled first, the opposite buy order is cancelled and opens a short position.
...
Parameters that are not prompted when creating the strategy but can be configured to make the bot more customizable.

- [How to use `config` command](/command-ref/#config)
