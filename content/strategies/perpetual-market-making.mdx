...
<Callout type="note" body="Paper trading is supported, enter `(perpetual_connector_name)_testnet` when prompted for derivative name." />
...
<Callout type="note" body="Initial margin deposits are calculated using the leverage as entered by the trader." />
