...
Generally, position modes are available when using a perpetual connector :
...
In `One-way` mode, users can have one position open after the orders have been filled regardless of `position_management`. Refer to the example below:
...
Unlike one-way mode, it is possible to have a long and short position open concurrently without affecting the size of one another.
...
Places a short or long position exit price from a position entry price determined by the spreads that you set. After an order is filled, a position is opened immediately. The parameters under Profit taking are `long_profit_taking_spread` and `short_profit_taking_spread`.

<br />

`long_profit_taking_spread`

It is the spread above the entry price at which a short position is placed to reduce position.
...
It is the spread below the entry price at which a long position is placed to reduce position.
...
After the position has been opened, it will now create two open orders that will determine if it will close the current open position or open another position.

![](/img/open-orders2.png)

If by chance, the order for `open long` gets filled, it will now open a long position, and you now have two open orders.

![](/img/two-positions.png)
...
An order would be placed to close the position immediately if the bid book’s top order’s price goes higher than the entry price of a short position or the top order of the ask book goes lower than the entry price of a long position. Trailing would begin if the current price is beyond or equal to the set `ts_activation_spread`. Setting this parameter to zero would start the trailing asap.

<Callout
  type="note"
  body="`Trailing_stop` can stop-loss immediately when a loss is detected, even when trailing has not begun.
Unlike `Profit_taking`, take profit feature places a limit order at a profitable price from position entry price, which is determined by the spreads for each take profit side."
/>
...
This feature gives users the option to select the order type for closing positions when using the trailing stop feature.
...
Once a profitable exit price is detected, no stop-loss order should be placed. An exception will be made for cases where the market swings swiftly to an unprofitable price within ticks or if the configured trailing stop parameters are too low.
