...
<Prompt prompt="How much leverage do you want to use? (Binance Perpetual supports up to 75X for most pairs) >>>" />
...
<Prompt prompt="Which position mode do you want to use? (One-way/Hedge) >>>" />
...
<Prompt prompt="What is the amount of [trading pair] per order? >>>" />
...
<Prompt prompt="How would you like to manage your positions? (Profit_taking/Trailing_stop) >>>" />
...
<Prompt prompt="At what spread from the entry price do you want to place a short order to reduce position? (Enter 1 for 1%) >>>" />
...
<Prompt prompt="At what spread from the entry price do you want to place a long order to reduce position? (Enter 1 for 1%) >>>" />
...
<Prompt prompt="At what spread from the position entry price do you want the bot to start trailing? (Enter 1 for 1%) >>>" />
...
<Prompt prompt="At what spread away from the trailing peak price do you want positions to remain open before they're closed? (Enter 1 for 1%) >>>" />
...
<Prompt prompt="What order type do you want to use for closing positions? (LIMIT/MARKET) >>>" />
