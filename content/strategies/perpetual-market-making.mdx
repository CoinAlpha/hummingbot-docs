...
<Callout
  type="tip"
  body="Users can test how this strategy works without risking real funds by connecting their Binance Futures Testnet account to Hummingbot. We also recommend to run the bot side-by-side with the exchange to help learning more about the strategy."
/>
...
Initially, a limit buy and sell order are created based on `bid_spread` and `ask_spread` settings. This determines the entry price of the position to be opened when these orders are filled.
...
If the buy order is filled then the opposite sell order is cancelled and a long position is opened. On the other hand, if your sell order is filled first then the opposite buy order is cancelled and opens a short position.
