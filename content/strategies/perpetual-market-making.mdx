...
Generally, two(2) position modes are available when using a perpetual connector : 1. One-way mode : In this mode, only one active position(long or short) per trading pair is possible. Trying to open a second position would result in increasing or decreasing the size of the position depending of the side of the second position. 2. Hedge mode: Unlike one-way mode, it is possible to have a long and short position open concurrently without affecting the size of one another.
