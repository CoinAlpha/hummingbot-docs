...
<Callout
  type="note"
  body="Enabling and setting the time parameters for `is_time_span_execution` will automatically ignore the parameters set for `order_delay_time`. 
  
  Calculation formula of `is_time_span_execution` is creation_delta = (span_end_time - span_start_time)/initial_number_of_orders_to_create."
/>

## Using `is_time_span_execution`

Enabling and setting the time parameters for `is_time_span_execution` will automatically ignore the parameters set for `order_delay_time`.

Calculation: `creation_delta = (span_end_time - span_start_time)/initial_number_of_orders_to_create`

while `initial_number_of_orders_to_create` = `target_asset_amount` / `order_step_size`.

### Sample TWAP Strategy Config file
...
start_datetime: '2021-08-12 10:15:00'

# Date and time the strategy should stop running
# Only valid if is_time_span_execution is True
end_datetime: '2021-08-12 10:35:00'
...
target_asset_amount = 200
order_step_size = 50
initial_number_of_orders_to_create = 4
is_time_span_execution = 20 minutes
Order execution time will be every 5 minutes.
```
