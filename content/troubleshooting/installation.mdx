---
title: Installation
description: How to fix installation errors.
---

### Hummingbot has 3 installation options, these are Binary, Docker and Source build.

The most common used when starting with Market making are with Binary. This build would normally require you to download the installation file from [Hummingbot.io](https://hummingbot.io/download/) and the easiest way for users to setup and run Hummingbot. Binary setup packages are released with every Hummingbot release starting from v0.18.

When running multiple bots, we normally suggest to make use of Docker build. This can be use locally or on cloud services. Docker would allow you to create containers and run multiple instances. Docker is also required when running Hummingbot-gateway to connect to Protocol connectors

For developers, we normally suggest to make use of Hummingbot on Source build where they can integrate custom scripts/strategies.
